...
**字符集强制 utf8mb4：** MySQL 的 `utf8` 是 3 字节子集，中文会乱码。所有层面必须使用 `utf8mb4`：

- **数据库**：`CREATE DATABASE litemes CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;`
- **JDBC URL**：`characterEncoding=UTF-8&connectionCollation=utf8mb4_unicode_ci`（已配置在 application.yml）
- **建表**：`DEFAULT CHARSET=utf8mb4 COLLATE utf8mb4_unicode_ci`（schema.sql 已统一）

**默认管理员：** 系统无预置用户，需手动插入：
```sql
INSERT INTO user (username, password, realName, status, createdAt)
