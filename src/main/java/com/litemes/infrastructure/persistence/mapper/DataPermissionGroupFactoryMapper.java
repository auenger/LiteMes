package com.litemes.infrastructure.persistence.mapper;

import com.baomidou.mybatisplus.core.mapper.BaseMapper;
import com.litemes.domain.entity.DataPermissionGroupFactory;
import org.apache.ibatis.annotations.Mapper;

/**
 * MyBatis-Plus Mapper for DataPermissionGroupFactory entity.
 * Provides standard CRUD operations via BaseMapper.
 */
@Mapper
public interface DataPermissionGroupFactoryMapper extends BaseMapper<DataPermissionGroupFactory> {
}
