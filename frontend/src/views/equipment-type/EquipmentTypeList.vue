  ...
  <div class="h-full flex flex-col space-y-3">
    ...
    <div class="bg-card p-3 border border-border-color shadow-sm rounded-sm shrink-0">
      <el-form :inline="true" :model="query" size="default" class="flex flex-wrap gap-y-3 items-center !mb-0">
        <el-form-item label="设备类型编码" class="!mb-0">
          <el-input v-model="query.typeCode" placeholder="设备类型编码" clearable class="!w-40" @keyup.enter="search" />
        </el-form-item>
        <el-form-item label="设备类型名称" class="!mb-0">
          <el-input v-model="query.typeName" placeholder="设备类型名称" clearable class="!w-40" @keyup.enter="search" />
        </el-form-item>
    ...
    <div class="flex-1 bg-card border border-border-color shadow-sm rounded-sm flex flex-col overflow-hidden">
      <div class="px-4 py-2.5 border-b border-border-color flex justify-between items-center shrink-0">
        <div class="flex gap-2">
          <el-button type="primary" :icon="Plus" @click="openCreateDialog">新建设备类型</el-button>
      ...
      <div class="flex-1 p-2.5 overflow-hidden">
        <el-table :data="types" border stripe height="100%" v-loading="loading">
          <el-table-column v-if="isColumnVisible('typeCode')" prop="typeCode" label="设备类型编码" min-width="140" />
          <el-table-column v-if="isColumnVisible('typeName')" prop="typeName" label="设备类型名称" min-width="140" />
    ...
    <!-- Create/Edit Dialog -->
    <el-dialog v-model="dialogVisible" :title="isEdit ? '编辑设备类型' : '新建设备类型'" width="480px" @close="closeDialog">
      <el-form label-width="120px">
        <el-form-item label="设备类型编码" required>
          <el-input v-model="form.typeCode" :disabled="isEdit" placeholder="请输入设备类型编码" />
        </el-form-item>
    ...
    <el-dialog v-model="deleteDialogVisible" title="确认删除" width="400px" @close="closeDeleteDialog">
      <p>确定要删除设备类型 "{{ deleteTarget?.typeName }}" ({{ deleteTarget?.typeCode }}) 吗？</p>
      <p class="text-gray-400 text-sm mt-2">已被设备型号引用的类型不可删除。删除后不可恢复。</p>
      <template #footer>
...
const types = ref<EquipmentTypeDto[]>([]);
const total = ref(0);
const loading = ref(false);
const submitting = ref(false);
const formError = ref('');

const query = reactive<EquipmentTypeQueryParams>({
  typeCode: '',
  typeName: '',
  status: undefined,
  page: 1,
...
const { columns, toggleColumn, resetSettings } = useTableSettings('equipment-type-list', defaultColumns);
...
const auditLogTableName = ref('equipment_type');
const auditLogRecordId = ref(0);

function openAuditLog(item: EquipmentTypeDto) {
  auditLogTableName.value = 'equipment_type';
...
const deleteTarget = ref<EquipmentTypeDto | null>(null);

async function fetchList() {
  loading.value = true;
  try {
    const res = await listEquipmentTypes(query);
    if (res.code === 200 && res.data) {
      types.value = res.data.records;
      total.value = res.data.total;
    }
  } catch (e) {
    console.error('Failed to fetch equipment type list', e);
  } finally {
    loading.value = false;
  }
}
...
function openEditDialog(item: EquipmentTypeDto) {
  isEdit.value = true;
...
async function submitForm() {
  if (!form.typeCode && !isEdit.value) {
    formError.value = '设备类型编码不能为空';
    return;
  }
  if (!form.typeName) {
    formError.value = '设备类型名称不能为空';
    return;
  }
  ...
  try {
    if (isEdit.value && editingId.value) {
      await updateEquipmentType(editingId.value, {
...
function confirmDelete(item: EquipmentTypeDto) {
...
async function doDelete() {
  ...
  submitting.value = true;
  try {
    await deleteEquipmentType(deleteTarget.value.id);
    closeDeleteDialog();
    fetchList();
  } catch (e: any) {
...
async function toggleStatus(item: EquipmentTypeDto) {
  const newStatus = item.status === 1 ? 0 : 1;
  try {
    await updateEquipmentTypeStatus(item.id, newStatus);
    fetchList();
  } catch (e: any) {
