  ...
  <div class="uom-page">
    <div class="page-header">
      ...
      <div class="header-actions">
        ...
        <button class="btn btn-primary" @click="openCreateDialog">新建单位</button>
      </div>
    </div>
    ...
    <table class="table">
      <thead>
        <tr>
          <th v-if="isColumnVisible('uomCode')">单位编码</th>
          <th v-if="isColumnVisible('uomName')">单位名称</th>
          <th v-if="isColumnVisible('precision')">精度</th>
      ...
      <tbody>
        ...
        <tr v-for="uom in uoms" :key="uom.id">
          <td v-if="isColumnVisible('uomCode')">{{ uom.uomCode }}</td>
          <td v-if="isColumnVisible('uomName')">{{ uom.uomName }}</td>
          <td v-if="isColumnVisible('precision')">{{ uom.precision ?? '-' }}</td>
          <td v-if="isColumnVisible('status')">
            <span :class="['status-badge', uom.status === 1 ? 'status-enabled' : 'status-disabled']">
              {{ uom.status === 1 ? '启用' : '禁用' }}
            </span>
          </td>
          <td v-if="isColumnVisible('createdBy')">{{ uom.createdBy || '-' }}</td>
          <td v-if="isColumnVisible('createdAt')">{{ formatDate(uom.createdAt) }}</td>
          <td class="actions">
            <button class="btn btn-sm" @click="openEditDialog(uom)">编辑</button>
            <button
              v-if="uom.status === 1"
            ...
            <button class="btn btn-sm btn-danger" @click="confirmDelete(uom)">删除</button>
            <button class="btn btn-sm btn-info" @click="openAuditLog(uom)">变更履历</button>
          </td>
        </tr>
      </tbody>
    </table>
    ...
    <div v-if="dialogVisible" class="dialog-overlay" @click.self="closeDialog">
      <div class="dialog">
        <div class="dialog-header">
          <h3>{{ isEdit ? '编辑单位' : '新建单位' }}</h3>
          <button class="dialog-close" @click="closeDialog">&times;</button>
        </div>
        <div class="dialog-body">
          <div class="form-group">
            <label>单位编码 <span class="required">*</span></label>
            <input
    ...
    <div v-if="deleteDialogVisible" class="dialog-overlay" @click.self="closeDeleteDialog">
      <div class="dialog dialog-sm">
        ...
        <div class="dialog-body">
          ...
          <p class="text-muted">已被换算比例引用的单位不可删除。删除后不可恢复。</p>
        </div>
        <div class="dialog-footer">
